...
public class MyViewDz4 extends View {
    ...
    public void initialize (){
        ...
        int color2= (ContextCompat.getColor( getContext(), R.color.dz4Black ));
        ...
        int color3= (ContextCompat.getColor( getContext(), R.color.dz4Black ));
...
//        shadowPaint.setShadowLayer(5.0f, 10.0f, 10.0f, R.color.dz2Red);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        cx=mWidth/2;
        cy=mHeight/2;
        radius =Math.min(mHeight,mWidth)/4;
        stopYline = cy - radius;
        cYline = cy - radius + 20;
...
//        canvas.drawCircle(cx,cy,10,myPaint3);

        for (int i=1; i<=12;i++) {
            canvas.rotate(30,cx,cy);
            canvas.drawLine(cx, cYline, cx, stopYline, myPaint);

            canvas.drawText(String.valueOf(i), cx, cYline+15, shadowPaint);
           }
        canvas.restore();
...
//        canvas.drawText(String.valueOf(mMinutes), cx, cy, shadowPaint);
        canvas.drawLine(cx,cy,cx,cy-radius/2,myPaint);;  //Minutes
        canvas.restore();
